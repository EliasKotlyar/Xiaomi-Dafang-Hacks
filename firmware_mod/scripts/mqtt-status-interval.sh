...
do
  ...
  /system/sdcard/bin/mosquitto_pub.bin -h "$HOST" -p "$PORT" -u "$USER" -P "$PASS" -t "${TOPIC}"/ftp_server ${MOSQUITTOPUBOPTS} ${MOSQUITTOOPTS} -r -m "$(ftp_server status)"
