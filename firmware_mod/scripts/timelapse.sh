...
[ -f "$TIMELAPSE_CONF" ] && . "$TIMELAPSE_CONF" 2>/dev/null
[ -z "$TIMELAPSE_INTERVAL" ] &&  TIMELAPSE_INTERVAL=2.0
...
while true; do
    ...
    [ $SAVE_DIR_PER_DAY -eq 1 ] && SAVE_DIR="$BASE_SAVE_DIR/$(date +%Y-%m-%d)"
    ...
    if [ $TIMELAPSE_DURATION -gt 0 ]; then
        ...
        [ $(($TIMELAPSE_DURATION * 60)) -le $elapsed ] && break
    fi
done
